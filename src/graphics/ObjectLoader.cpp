...
std::vector<Vertex> ObjectLoader::loadOBJ(const char* file_name)
{
	//Vertex portions
...
std::vector<unsigned char*>ObjectLoader::loadCubemap(std::vector<std::string> filepaths, int& width, int& height, int& channels)
{
	std::vector<unsigned char*> vektori;
	for (unsigned int i = 0; i < filepaths.size(); i++)
	{
		std::string filepath = filepaths[i];
		stbi_set_flip_vertically_on_load(false);
		unsigned char* data = stbi_load(filepath.c_str(), &width, &height, &channels, 0);
		vektori.push_back(data);
	}
